...
class MixedContentConfig(object):
    def __init__(self):
        self.selection_pattern = \
              '%(subresource)s/%(source_context)s.%(delivery_type)s/%(delivery_value)s/' + \
              '%(origin)s.%(redirection)s.%(source_protocol)s'

        self.test_file_path_pattern = 'gen/' + self.selection_pattern + '.html'

        self.test_description_template = 'The mixed content should be %(expected)s for %(subresource)s to %(origin)s origin and %(redirection)s redirection from %(source_protocol)s context.'
